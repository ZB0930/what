...
class LSTM2(nn.Module): 
   def __init__(self, input_size, hidden_size, layers=2, sequences=True):
       ...
       if layers > 1:
           for i in range(1, layers):
               ...
               #setattr()函数的作用是，把lay变成self.lay 'i' ,如果layers = 3,该行代码等同于：
               #self.lay0 = LSTMCell(input_size,hidden_size) 
   ...
   def forward(self, input, initial_states=None):
       ...
       for t in range(length):
            ...
            for l in range(self.layers):
                ...
                states[l] = hc   #如上图所示，左面的输出(h,c)做右面的状态信息输入
                x = hc[0]   #如上图所示，下面的LSTMcell的输出h做上面的LSTMcell的输入
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.LSTM = LSTM2(input_size=emb_size, hidden_size=n_hidden)
